...
const releaseBuild = process.argv.includes("--release")
  || String(process.env.CARD_CRUNCH_BUILD_FLAVOR || "").trim().toLowerCase() === "release";
...
const defaultWebRedirectUri = releaseBuild
  ? "https://card-crunch-release.vercel.app/auth/callback"
...
const platformConfig = Object.freeze({
  ...
  webRedirectUri: String(process.env.VITE_STL_WEB_REDIRECT_URI || defaultWebRedirectUri).trim()
});
validateSTLPlatformConfig(platformConfig, { hostname: new URL(platformConfig.webRedirectUri).hostname });
await writeFile(
...
await writeFile(
  resolve(output, "build-config.js"),
  `globalThis.__CARD_CRUNCH_BUILD_CONFIG__ = Object.freeze(${JSON.stringify({
    flavor: releaseBuild ? "release" : "development",
    potsOnly: releaseBuild,
    releaseChannel: releaseBuild ? "google-play" : "development",
    webUrl: releaseBuild ? "https://card-crunch-release.vercel.app" : "https://card-crunch.vercel.app"
  })});\n`,
  "utf8"
);

console.log(`Built Card Crunch ${releaseBuild ? "release" : "development"} web assets in ${output}`);
